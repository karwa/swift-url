...
extension WebURLTests {
  ...
  func testHostRelativeJoin() {
    ...
    do {
      ...
      XCTAssertEqual(url_r, url.join("/usr/lib/swift"))
    }
    do {
      let url = WebURL("http://example.com/p1/p2/p3?query=good#fragggggment")!
      let url_r = url.join(hostRelative: "//notahost/test")
      XCTAssertEqual(url_r?.serialized, "http://example.com//notahost/test")
      XCTAssertEqual(url_r?.path, "//notahost/test")
      XCTAssertEqual(url_r, url.join("/.//notahost/test"))
    }
    ...
    do {
      ...
      XCTAssertEqual(url_r, url.join("file:D:/hello/"))
    }
    ...
    do {
      ...
      XCTAssertEqual(base.join("D:/Media")?.serialized, "d:/Media")
      XCTAssertEqual(base.join(hostRelative: "D:/Media")?.serialized, "file:///D:/Media")
    }
    do {
