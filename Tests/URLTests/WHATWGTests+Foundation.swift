         ...
         for item in array {
             if let sectionName = item as? String {
                 ...
                 report.recordTest { report in
                    let _parserResult = URL(string: expected.base!).flatMap { URL(string: expected.input!, relativeTo: $0) }?.jsModel
                     // Capture test data.
...
struct JSObjectModel {
    ...
    public var port: UInt16? {
        get { return url.port.flatMap { UInt16(exactly: $0) } }
    }
