...
extension OldURLParser {
  ...
  struct Components: Equatable, Hashable, Codable {
    fileprivate final class Storage: Equatable, Hashable, Codable {
      var scheme: OldURLParser.Scheme
      var username: String
      var password: String
      var host: OldURLParser.Host?
      var port: UInt16?
      var path: [String]
      var query: String?
      var fragment: String?
      ...
      init(
        scheme: OldURLParser.Scheme, username: String, password: String, host: OldURLParser.Host?,
      ...
      ///
      func copyAuthority(from other: OldURLParser.Components.Storage) {
    ...
    init(
      scheme: OldURLParser.Scheme = .other(""), username: String = "", password: String = "",
      host: OldURLParser.Host? = nil,
...
extension OldURLParser.Components {
  ...
  /// ```
  ///
  var queryParameters: OldURLParser.QueryParameters? {
    get { return query?._withUTF8 { OldURLParser.QueryParameters(parsingUTF8: $0) } }
...
extension OldURLParser.Components {
  ...
  ///
  @discardableResult
  mutating func modify<S>(_ input: S, stateOverride: OldURLParser.ParserState?) -> Bool where S: StringProtocol {
    ensureUnique()
    return input._withUTF8 {
      var buffer = [UInt8]()
      buffer.reserveCapacity(32)
      var callback = IgnoreValidationErrors()
      return OldURLParser._parse(
...
extension OldURLParser {
  ...
  public struct ValidationError: Equatable {
    ...
    enum AnyHostParserError: Equatable, CustomStringConvertible {
      ...
      case hostParserError(OldURLParser.Host.ValidationError)
...
public protocol URLParserCallback: IPv6AddressParserCallback, IPv4ParserCallback {
  mutating func validationError(_ error: OldURLParser.ValidationError)
}
...
extension URLParserCallback {
  ...
  public mutating func validationError(ipv4 error: IPv4Address.ValidationError) {
    let wrapped = OldURLParser.ValidationError.AnyHostParserError.ipv4AddressError(error)
    validationError(.hostParserError(wrapped))
  }
  public mutating func validationError(ipv6 error: IPv6Address.ValidationError) {
    let wrapped = OldURLParser.ValidationError.AnyHostParserError.ipv6AddressError(error)
    validationError(.hostParserError(wrapped))
  }
  // Other host-parser errors.
  public mutating func validationError(hostParser error: OldURLParser.Host.ValidationError) {
    let wrapped = OldURLParser.ValidationError.AnyHostParserError.hostParserError(error)
    validationError(.hostParserError(wrapped))
  }
  public mutating func validationError(opaqueHost error: OpaqueHost.ValidationError) {
    let wrapped = OldURLParser.ValidationError.AnyHostParserError.opaqueHostError(error)
    validationError(.hostParserError(wrapped))
  }
}
extension OldURLParser.ValidationError {
...
public struct IgnoreValidationErrors: URLParserCallback {
  ...
  @inlinable @inline(__always) public mutating func validationError(_ error: OldURLParser.ValidationError) {}
}

/// A `URLParserCallback` which stores the last reported validation error.
///
public struct LastValidationError: URLParserCallback {
  public var error: OldURLParser.ValidationError?
  @inlinable @inline(__always) public init() {}
  @inlinable @inline(__always) public mutating func validationError(_ error: OldURLParser.ValidationError) {
    self.error = error
  }
...
public struct CollectValidationErrors: URLParserCallback {
  public var errors: [OldURLParser.ValidationError] = []
  ...
  private static func _parse_stateMachine<Source, Callback>(
    ...
    inputLoop: while true {
      ...
      case .scheme:
        ...
        assert(c == .colon)
        let newScheme = OldURLParser.Scheme.parse(asciiBytes: buffer)
        if stateOverride != nil {
      ...
      case .hostname, .host:
        ...
        case .colon? where flag_squareBracket == false:
          ...
          guard let parsedHost = buffer.withUnsafeBufferPointer({
              OldURLParser.Host.parse($0, isNotSpecial: url.scheme.isSpecial == false, callback: &callback)
          }) else {
            return false
          }
          url.host = parsedHost
      ...
      case .fileHost:
        ...
        case .forwardSlash?, .backslash?, .questionMark?, .numberSign?:  // or endIndex.
          if stateOverride == nil, URLStringUtils.isWindowsDriveLetter(buffer) {
            ...
            // swift-format-ignore
            guard let parsedHost = buffer.withUnsafeBufferPointer({
                OldURLParser.Host.parse($0, isNotSpecial: false, callback: &callback)
            }) else {
              return false
            }
...
// swift-format-ignore
extension OldURLParser.ValidationError: CustomStringConvertible {
