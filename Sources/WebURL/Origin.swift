extension WebURLParser {
  ...
  public struct Origin {
    fileprivate enum Kind {
      case opaque
      case tuple(WebURLParser.Components)
    }
...
extension WebURLParser.Components {
  ...
  var origin: WebURLParser.Origin {
    switch scheme {
    case .http, .https, .ws, .wss, .ftp:
      return WebURLParser.Origin(kind: .tuple(self))
    case .file:
      return WebURLParser.Origin(kind: .opaque)
    case .other(let str) where str == "blob":
      ...
      return path.first.flatMap { WebURLParser.parse($0)?.origin } ?? WebURLParser.Origin(kind: .opaque)
    case .other(_):
      return WebURLParser.Origin(kind: .opaque)
    }
  }
