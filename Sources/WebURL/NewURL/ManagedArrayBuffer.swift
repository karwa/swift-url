...
struct AltManagedBufferReference<Header: ManagedBufferHeader, Element> {
 ...
 private final class _Storage: ManagedBuffer<Header, Element> {
  ...
  static func newBuffer(minimumCapacity: Int, initialHeader: Header) -> Self {
   ...
   // TODO: Is there a way to detect when this is available?
   buffer.header.capacity = buffer.capacity // minimumCapacity
  ...
  static func newBuffer(
    ...
    let buffer = Self.create(minimumCapacity: minimumCapacity, makingHeaderWith: { newBuffer in
      // TODO: Is there a way to detect when this is available?
      let allocatedCapacity = newBuffer.capacity // minimumCapacity
      return newBuffer.withUnsafeMutablePointerToElements {
        headerConstructor(UnsafeMutableBufferPointer(start: $0, count: allocatedCapacity))
      }
    })
 ...
 ///   call to `body`.
 @inlinable func withUnsafeMutablePointerToHeader<R>(_ body: (UnsafeMutablePointer<Header>) throws -> R) rethrows -> R {
 ...
 /// - Note: This pointer is valid only for the duration of the
 ///   call to `body`.
 @inlinable func withUnsafeMutablePointerToElements<R>(_ body: (UnsafeMutablePointer<Element>) throws -> R) rethrows -> R {
 ...
 ///   call to `body`.
 @inlinable func withUnsafeMutablePointers<R>(_ body: (UnsafeMutablePointer<Header>, UnsafeMutablePointer<Element>) throws -> R) rethrows -> R {
 ...
 func moveToNewBuffer(minimumCapacity: Int) -> Self {
  let c = count
  precondition(minimumCapacity >= c)
  ...
  newBuffer.withUnsafeMutablePointers { destHeader, destElems in
   self.withUnsafeMutablePointers { srcHeader, srcElems in
    destElems.moveInitialize(from: srcElems, count: c)
    srcHeader.pointee.count = 0
   }
   destHeader.pointee.count = c
  }
  assert(newBuffer.count == c)
  assert(self.count == 0)
  return newBuffer
 }
 ...
 func copyToNewBuffer(minimumCapacity: Int) -> Self {
  ...
  newBuffer.withUnsafeMutablePointers { destHeader, destElemens in
   self.withUnsafeMutablePointerToElements { srcElems in
    destElemens.initialize(from: srcElems, count: c)
   }
   destHeader.pointee.count = c
  }
  assert(newBuffer.count == c)
...
extension ManagedArrayBuffer: MutableCollection {
 ...
 subscript(position: Int) -> Element {
  ...
  set {
   precondition(position >= startIndex && position < endIndex, "Index out of bounds")
   ensureUnique()
   return storage.withUnsafeMutablePointerToElements { $0.advanced(by: position).assign(repeating: newValue, count: 1) }
  }
  _modify {
   precondition(position >= startIndex && position < endIndex, "Index out of bounds")
   ensureUnique()
...
extension ManagedArrayBuffer: AltRangeReplaceableCollection {
 ...
 mutating func reserveCapacity(_ minimumCapacity: Int) {
  ...
  if _slowPath(!isUnique || storage.capacity < minimumCapacity) {
   ...
   storage = isUnique ? storage.moveToNewBuffer(minimumCapacity: newCapacity) :
    storage.copyToNewBuffer(minimumCapacity: newCapacity)
  }
 ...
 @discardableResult
 mutating func replaceSubrange<C>(_ subrange: Range<Int>, with newElements: C) -> Range<Int> where C : Collection, Self.Element == C.Element {
  let isUnique = storage.isKnownUniqueReference()
  let result = storage.withUnsafeMutablePointerToElements { elems in
   return replaceElements(
    ...
    with: newElements,
    isUnique: isUnique,
    storageConstructor: { StorageHolder(bufferReference: .init(minimumCapacity: $0, initialHeader: storage.header)) }
   )
  }
...
extension ManagedArrayBuffer {
  ...
  ///
  mutating func unsafeAppend<T>(uninitializedCapacity: Int, initializingWith initializer: (UnsafeMutableBufferPointer<Element>)->T) -> T {
